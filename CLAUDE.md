...
| Shared cached corpus prefix + UCF section targeting | `bidpilot/prompting.py` — `split_for_cache` head MUST stay byte-identical across stages or every call is a silent miss; `STAGE_SECTIONS` decides what an oversized corpus spends its budget on |
